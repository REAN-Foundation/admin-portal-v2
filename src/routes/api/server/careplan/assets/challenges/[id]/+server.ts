...
import { deleteChallenges, getChallengesById, updateChallenges } from "$routes/api/services/careplan/assets/challenges";
...
export const DELETE = async (event: RequestEvent) => {
    ...
    try {
        console.log("Inside the challenages delete server endpoint")
        ...
        const response = await deleteChallenges(
            sessionId,
            id
        );
...
export const GET = async (event: RequestEvent) => {
    try {
        ...
        const response = await getChallengesById(sessionId, id);

        return ResponseHandler.success(response);
    } catch (error) {
        console.error("Error challenges:", error);
        return ResponseHandler.handleError(500, null, error);
    }
};
...
export const PUT = async (event: RequestEvent) => {
    try {
        console.log("Inside challenges PUT endpoints");
        const sessionId = event.locals?.sessionUser?.sessionId;
        ...
        const data: ChallengesUpdateModel = await request.json();
        ...
        const response = await updateChallenges(sessionId,
            audioId,
            data.Name,
            data.Description,
            data.Tags,
