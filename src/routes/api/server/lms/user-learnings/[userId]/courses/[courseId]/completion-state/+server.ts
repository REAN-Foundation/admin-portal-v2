...
import { getCourseCompletionState } from '$routes/api/services/lms/user-learnings';
...
export const GET = async (event: RequestEvent) => {
	try {
		...
		const response = await getCourseCompletionState(sessionId, userId, courseId);
		return ResponseHandler.success(response);
	} catch (error) {
		console.error('Error fetching course completion state:', error);
