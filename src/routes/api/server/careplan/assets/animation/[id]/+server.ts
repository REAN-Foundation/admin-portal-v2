...
import {
	...
	getAnimationById,
	updateAnimation
} from '../../../../../services/careplan/assets/animation';
...
export const DELETE = async (event: RequestEvent) => {
	try {
		...
		const animationId = event.params.animationId;

		const response = await deleteAnimation(sessionId, animationId);
...
export const PUT = async (event: RequestEvent) => {
	try {
		console.log('Inside Animation server PUT endpoints');
		const sessionId = event.locals?.sessionUser?.sessionId;
