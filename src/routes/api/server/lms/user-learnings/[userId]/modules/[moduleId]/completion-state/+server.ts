...
import { getModuleCompletionState } from '$routes/api/services/lms/user-learnings';
...
export const GET = async (event: RequestEvent) => {
	try {
		...
		const response = await getModuleCompletionState(sessionId, userId, moduleId);
		return ResponseHandler.success(response);
	} catch (error) {
		console.error('Error fetching module completion state:', error);
