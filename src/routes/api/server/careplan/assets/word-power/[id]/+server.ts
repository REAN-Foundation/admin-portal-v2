...
import {
	...
	getWordPowerById,
	updateWordPower
} from '$routes/api/services/careplan/assets/word-power';
...
export const DELETE = async (event: RequestEvent) => {
	try {
		...
		const wordPowerId = event.params.assetId;
		// const selectAsset = event.params.selectAsset;

		const response = await deleteWordPower(sessionId, wordPowerId);
...
export const GET = async (event: RequestEvent) => {
	try {
		...
		const response = await getWordPowerById(sessionId, id);

		return ResponseHandler.success(response);
	} catch (error) {
		console.error('Error fetching Word Power:', error);
		return ResponseHandler.handleError(500, null, error);
	}
};
...
export const PUT = async (event: RequestEvent) => {
	try {
		console.log('Inside Word Power server PUT endpoints');
		const sessionId = event.locals?.sessionUser?.sessionId;
		...
		const data: WordPowerUpdateModel = await request.json();
