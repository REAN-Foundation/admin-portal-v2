...
export const DELETE = async (event: RequestEvent) => {
	try {
		...
		const reflectionId = event.params.assetId;

		const response = await deleteReflection(sessionId, reflectionId);
...
export const GET = async (event: RequestEvent) => {
	try {
		...
		const response = await getReflectionById(sessionId, id);
...
export const PUT = async (event: RequestEvent) => {
	try {
		...
		const data: ReflectionUpdateModel = await request.json();
