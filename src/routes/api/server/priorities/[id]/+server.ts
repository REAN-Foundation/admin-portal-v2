...
export const DELETE = async (event: RequestEvent) => {
    ...
    try {
        ...
        const response = await deletePriority(
            sessionId,
            id
        );
...
export const PUT = async (event: RequestEvent) => {
    try {
        console.log("Inside priorities server PUT endpoints");
        const sessionId = event.locals?.sessionUser?.sessionId;
        ...
        const data: PrioritiesTypeUpdateModel = await request.json();
        ...
        const response = await updatePriority(sessionId, priorityId, data.Type, data.Tags);
