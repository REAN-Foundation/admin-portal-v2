...
import { deleteTenant, getTenantById, updateTenant } from '../../../services/rean-care/tenants';
...
export const DELETE = async (event: RequestEvent) => {
	try {
		...
		const response = await deleteTenant(sessionId, id);
...
export const GET = async (event: RequestEvent) => {
	try {
		...
		const response = await getTenantById(sessionId, id);
...
export const PUT = async (event: RequestEvent) => {
	try {
		console.log('Inside tenants server PUT endpoints');
		const sessionId = event.locals?.sessionUser?.sessionId;
		...
		const data: TenantsUpdateModel = await request.json();
