<script lang="ts">
	...
	import type { ChallengesCreateModel } from '$lib/types/challenges.types.js';
	import { createOrUpdateSchema } from '$lib/validation/challenges.schema.js';
	...
	// Get asset type from URL params or default to 'Challenge'
	const assetType = 'Challenge';
	
	const assetRoute = `/users/${userId}/careplan/assets?assetType=${assetType}`;
	const createRoute = `/users/${userId}/careplan/assets/challenges/create?assetType=${assetType}`;
	...
	const handleSubmit = async (event: Event) => {
		try {
			...
			const challengesCreateModel: ChallengesCreateModel = {
				Name: name,
			...
			console.log(challengesCreateModel);
			const res = await fetch(`/api/server/careplan/assets/challenges`, {
				method: 'POST',
				body: JSON.stringify(challengesCreateModel),
...
<div class="p-6">
	<form onsubmit={async (event) => (promise = handleSubmit(event))}>
		<div class="form-headers">
			<h2 class="form-titles">Create Challenge</h2>
