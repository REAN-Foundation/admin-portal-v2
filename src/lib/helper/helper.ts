...
export class Helper {
	...
	static truncateText = (text: string, numChars: number) => {
		const txt = text.length > numChars ? text.substring(0, numChars - 3) + '...' : text;
		return txt;
	};

	static hasProperty = (obj: ObjectConstructor, prop: PropertyKey) => {
		return Object.prototype.hasOwnProperty.call(obj, prop);
	};

	static isEmpty = (obj: ObjectConstructor) => {
	...
	static toBase64 = async (file: File): Promise<string | ArrayBuffer| null> => {
	...
	static downloadAsInlineObjectUrl = (response: { Data: { Buffer: string } }) => {
	...
	static b64toBlob = async (base64Buffer:string, mimeType:string) => {
