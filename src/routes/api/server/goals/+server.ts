...
export const POST = async (event: RequestEvent) => {
	try {
		console.log('Inside goal server POST endpoints');
		...
		const data: GoalTypeCreateModel = await request.json();
		...
		const response = await createGoal(sessionId, data.Type, data.Tags);
