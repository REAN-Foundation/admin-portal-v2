...
import {
	...
	getHospitalById,
	updateHospital
} from '../../../services/reancare/hospitals';
...
export const GET = async (event: RequestEvent) => {
	try {
		...
		const response = await getHospitalById(sessionId, id);
...
export const PUT = async (event: RequestEvent) => {
	try {
		console.log('Inside hospital server PUT endpoints');
		const sessionId = event.locals?.sessionUser?.sessionId;
		...
		const data: HospitalUpdateModel = await request.json();
