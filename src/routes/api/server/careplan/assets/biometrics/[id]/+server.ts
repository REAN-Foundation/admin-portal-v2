...
export const DELETE = async (event: RequestEvent) => {
    ...
    try {
        ...
        const response = await deleteBiometrics(
            sessionId,
            id
        );
...
export const GET = async (event: RequestEvent) => {
    try {
        ...
        const response = await getBiometricsById(sessionId, id);
...
export const PUT = async (event: RequestEvent) => {
    try {
        console.log("Inside biometrics PUT endpoints");
        const sessionId = event.locals?.sessionUser?.sessionId;
        ...
        const data: BiometricsUpdateModel = await request.json();
        ...
        const response = await updateBiometrics(sessionId,
            biometricsId,
            data.Name,
            data.Description ?? '',            
            data.BiometricsType,
