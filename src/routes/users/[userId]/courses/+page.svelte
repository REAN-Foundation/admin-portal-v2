<script lang="ts">
	...
	let totalCoursesCount = $state(data.courses.TotalCount);
	$inspect('totalCoursesCount', totalCoursesCount);
	...
	let paginationSettings: PaginationSettings = $state({
		page: 0,
		limit: 10,
		size: totalCoursesCount,
		amounts: [10, 20, 30, 50]
	});

	$inspect('retrivedCourses', courses);
	...
	async function searchCourse(model) {
		try {
			...
			totalCoursesCount = searchResult.Data.Courses.TotalCount;
			paginationSettings.size = totalCoursesCount;

			courses = searchResult.Data.Courses.Items.map((item, index) => ({
				...item,
				index: index + 1
			}));
