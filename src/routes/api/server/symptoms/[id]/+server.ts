...
import { deleteSymptom, getSymptomById, updateSymptom } from '../../../services/reancare/symptoms';
...
export const GET = async (event: RequestEvent) => {
	try {
		...
		const response = await getSymptomById(sessionId, id);
...
export const PUT = async (event: RequestEvent) => {
	try {
		console.log("Inside symptom server PUT endpoints");
		const sessionId = event.locals?.sessionUser?.sessionId;
		...
		const symptonId = event.params.id;
		const request = event.request;
		const data: SymptomUpdateModel = await request.json();
