<script lang="ts">
	...
	const createRoute = `/users/${userId}/learning-journeys/enrollments/create/course`;

	const breadCrumbs = [
		{ name: 'Enrollments', path: enrollmentsRoute },
		{ name: 'Create Course Enrollment', path: createRoute }
	];
	...
	const handleSubmit = async (event: Event) => {
		...
		try {
			...
			console.error('Course enrollment creation failed:', error);
			toastMessage({
				HttpCode: 500,
...
<div class="p-6">
	<form onsubmit={async (event) => (promise = handleSubmit(event))}>
		<div class="form-headers">
			<h2 class="form-titles">Create Course Enrollment</h2>
