...
import { deletePhysiotherapy, getPhysiotherapyById, updatePhysiotherapy } from "$routes/api/services/careplan/assets/physiotherapy";
...
export const DELETE = async (event: RequestEvent) => {
    ...
    try {
        ...
        const response = await deletePhysiotherapy(
            sessionId,
            id
        );
...
export const GET = async (event: RequestEvent) => {
    try {
        ...
        const response = await getPhysiotherapyById(sessionId, id);

        return ResponseHandler.success(response);
    } catch (error) {
        console.error("Error physiotherapy:", error);
        return ResponseHandler.handleError(500, null, error);
    }
};
...
export const PUT = async (event: RequestEvent) => {
    try {
        console.log("Inside physiotherapy PUT endpoints");
        const sessionId = event.locals?.sessionUser?.sessionId;
        ...
        const data: PhysiotherapyUpdateModel = await request.json();
        ...
        const response = await updatePhysiotherapy(sessionId,
            exerciseId,
            data.Name,
            data.Description ?? '', 
