...
import { deleteExercise, getExerciseById, updateExercise } from "$routes/api/services/careplan/assets/exercises";
...
export const GET = async (event: RequestEvent) => {
    try {
        ...
        const response = await getExerciseById(sessionId, id);

        return ResponseHandler.success(response);
    } catch (error) {
        console.error("Error exercise:", error);
        return ResponseHandler.handleError(500, null, error);
    }
};
...
export const PUT = async (event: RequestEvent) => {
    try {
        console.log("Inside exercise PUT endpoints");
        const sessionId = event.locals?.sessionUser?.sessionId;
        ...
        const response = await updateExercise(sessionId,
            exerciseId,
            data.Name,
            data.Description ?? '',            
            data.ExerciseType,
