...
const createCheckupsSchema = zfd.formData({
...
export const actions = {
  createCheckupsAction: async (event: RequestEvent) => {
    ...
    type CheckupsSchema = z.infer<typeof createCheckupsSchema>;

    let result: CheckupsSchema = {};
    try {
      result = createCheckupsSchema.parse(formDataValue);
    ...
    const id = response.Data.Checkup.id;
    throw redirect(
      303,
      `/users/${userId}/careplan/assets/checkups/${id}/view`,
      successMessage(`Checkups created successfully!`),
      event
    );
  }
};
