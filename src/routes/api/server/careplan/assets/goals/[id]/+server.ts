...
import { deleteGoals, getGoalsById, updateGoals } from "$routes/api/services/careplan/assets/goals";
...
export const GET = async (event: RequestEvent) => {
    try {
        ...
        const response = await getGoalsById(sessionId, id);
...
export const PUT = async (event: RequestEvent) => {
    try {
        console.log("Inside goals PUT endpoints");
        const sessionId = event.locals?.sessionUser?.sessionId;
