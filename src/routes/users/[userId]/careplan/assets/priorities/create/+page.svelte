<script lang="ts">
	...
	import type { PriorityCreateModel } from '$lib/types/priority.types.js';
	import { createOrUpdateSchema } from '$lib/validation/priority.schema.js';
	...
	const handleSubmit = async (event: Event) => {
		try {
			...
			const priorityCreateModel: PriorityCreateModel = {
				Name: name,
			...
			const res = await fetch(`/api/server/careplan/assets/priorities`, {
				method: 'POST',
				body: JSON.stringify(priorityCreateModel),
			...
			if (response.HttpCode === 201 || response.HttpCode === 200) {
				...
				goto(`${priorityRoute}/${response?.Data?.id}/view`);
				return;
			}
